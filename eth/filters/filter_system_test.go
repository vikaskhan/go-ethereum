...
func TestBlockSubscription(t *testing.T) {
	...
	go func() { // simulate client
		...
		for i1 != len(chainEvents) || i2 != len(chainEvents) {
			select {
			case header := <-chan0:
				if chainEvents[i1].Block.Header().Hash() != header.Hash() {
					t.Errorf("sub0 received invalid hash on index %d, want %x, got %x", i1, chainEvents[i1].Block.Header().Hash(), header.Hash())
				}
				i1++
			case header := <-chan1:
				if chainEvents[i2].Block.Header().Hash() != header.Hash() {
					t.Errorf("sub1 received invalid hash on index %d, want %x, got %x", i2, chainEvents[i2].Block.Header().Hash(), header.Hash())
				}
				i2++
			}
		}
